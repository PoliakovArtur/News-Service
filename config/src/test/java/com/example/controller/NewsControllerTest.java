...
import static com.example.entity_examples.news.NewsJsonExamples.VALID_UPDATE_SPORT_NEWS_JSON;
import static com.example.entity_examples.news.NewsRequestExamples.INVALID_CREATE_SPORT_NEWS_REQUEST;
...
class NewsControllerTest extends MockMvcTest {
    ...
    void findById_shouldReturn200() throws Exception {
        when(facade.findById(1L)).thenReturn(FULL_SPORT_NEWS_RESPONSE);

        expectBodyFromGet(createUrl(NEWS_EP, ID), OK, FULL_SPORT_NEWS_RESPONSE);
    ...
    void findAll_shouldReturn200() throws Exception {
        when(facade.findAll(SIMPLE_NEWS_FILTER)).thenReturn(SHORT_SPORT_NEWS_RESPONSE_LIST);

        expectBodyFromGet(NEWS_EP, OK, SHORT_SPORT_NEWS_RESPONSE_LIST, "pageSize", "1", "pageNumber", "0");
    ...
    void save_shouldReturn201() throws Exception {
        when(facade.save(VALID_CREATE_SPORT_NEWS_REQUEST)).thenReturn(UPDATE_SPORT_NEWS_RESPONSE);

        expectBodyFromPost(NEWS_EP, VALID_CREATE_SPORT_NEWS_REQUEST, CREATED, UPDATE_SPORT_NEWS_RESPONSE);
        verify(facade, times(1)).save(VALID_CREATE_SPORT_NEWS_REQUEST);
    }

    @Test
    void saveWithEmptyTitle_shouldReturn400() throws Exception {
        expectMessageFromPost(NEWS_EP, INVALID_CREATE_SPORT_NEWS_REQUEST, BAD_REQUEST,
    ...
    void save_shouldReturn500() throws Exception {
        when(facade.save(VALID_CREATE_SPORT_NEWS_REQUEST)).thenThrow(new InternalError());

        expectMessageFromPost(NEWS_EP, VALID_CREATE_SPORT_NEWS_REQUEST, INTERNAL_SERVER_ERROR, "Внутренняя ошибка сервера");
    }

    @Test
    void updateById_shouldReturn200() throws Exception {
        when(facade.updateById(1L, VALID_UPDATE_SPORT_NEWS_REQUEST)).thenReturn(UPDATE_SPORT_NEWS_RESPONSE);

        expectBodyFromPut(createUrl(NEWS_EP, ID), VALID_UPDATE_SPORT_NEWS_JSON, OK, UPDATE_SPORT_NEWS_RESPONSE);
        verify(facade, times(1)).updateById(1L, VALID_UPDATE_SPORT_NEWS_REQUEST);
    }
    ...
    void updateByIdWithInvalidId_shouldReturn400() throws Exception {
        expectStatusFromPut(createUrl(NEWS_EP, WRONG_ID), VALID_UPDATE_SPORT_NEWS_JSON, BAD_REQUEST);
        verify(facade, times(0)).updateById(any(), any());
    }

    @Test
    void updateById_shouldReturn500() throws Exception {
        when(facade.updateById(1L, VALID_UPDATE_SPORT_NEWS_REQUEST)).thenThrow(new InternalError());

        expectMessageFromPut(createUrl(NEWS_EP, ID), VALID_UPDATE_SPORT_NEWS_JSON, INTERNAL_SERVER_ERROR, "Внутренняя ошибка сервера");
    }
